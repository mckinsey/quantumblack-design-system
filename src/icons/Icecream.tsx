// AUTO-GENERATED. Do not edit by hand.
// Source: Material Symbols (Sharp, 24dp, weight 300)
// Regenerate with: python3 ~/qbds-material-symbols-complete/wrapper-generator.py
import * as React from 'react';

import { cn } from '../lib/utils';

interface IconProps extends React.SVGProps<SVGSVGElement> {
  readonly className?: string;
}

export function Icecream({ className, ...props }: Readonly<IconProps>) {
  return (
    <svg
      className={cn('', className)}
      viewBox="0 -960 960 960"
      fill="currentColor"
      aria-hidden="true"
      {...props}>
      <path d="M482-83.08 305.54-423.85Q242.23-412 191.12-451.58 140-491.15 140-560q0-44.85 26.81-82.19 26.81-37.35 74.88-53.19 15.31-92.16 83.16-148.39Q392.69-900 480-900t155.15 56.23q67.85 56.23 83.16 148.39 48.07 15.84 74.88 53.19Q820-604.85 820-560q0 69.23-50.89 108.04-50.88 38.8-111.88 28.11L482-83.08ZM280-480q15 0 29.5-5t26.5-17l22-22 26 16q21 14 45.5 21t50.5 7q26 0 50.5-7t45.5-21l26-16 22 22q12 12 26.5 17t29.5 5q33 0 56.5-23.5T760-560q0-30-19-52.5T692-640l-30-4-2-32q-5-69-57-116.5T480-840q-71 0-123 47.5T300-676l-2 32-30 6q-30 6-49 27t-19 51q0 33 23.5 56.5T280-480Zm202 266 118-230-5.38-4.62q-27.47 13.93-56.81 21.27Q508.46-420 480-420q-25.46 0-54.31-6.77-28.84-6.77-57.54-21.85l-5.38 4.62L482-214Zm-2-446Z" />
    </svg>
  );
}
