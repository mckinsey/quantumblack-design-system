// AUTO-GENERATED. Do not edit by hand.
// Source: Material Symbols (Sharp, 24dp, weight 300)
// Regenerate with: python3 ~/qbds-material-symbols-complete/wrapper-generator.py
import * as React from 'react';

import { cn } from '../lib/utils';

interface IconProps extends React.SVGProps<SVGSVGElement> {
  readonly className?: string;
}

export function Psychology({ className, ...props }: Readonly<IconProps>) {
  return (
    <svg
      className={cn('', className)}
      viewBox="0 -960 960 960"
      fill="currentColor"
      aria-hidden="true"
      {...props}>
      <path d="M448.85-368.46h62.3l4.46-43.46q11.85-3 22.2-8.73 10.34-5.74 17.65-14.2l39.08 18.08 31.15-53-34.61-25.77q4.3-12.23 4.3-24.46t-4.3-24.46l34.61-25.77-31.15-53-39.08 18.08q-7.31-8.46-17.65-14.2-10.35-5.73-22.2-8.73l-4.46-43.46h-62.3l-4.46 43.46q-11.85 3-22.2 8.73-10.34 5.74-17.65 14.2l-39.08-18.08-31.15 53 34.61 25.77q-4.3 12.23-4.3 24.46t4.3 24.46l-34.61 25.77 31.15 53 39.08-18.08q7.31 8.46 17.65 14.2 10.35 5.73 22.2 8.73l4.46 43.46Zm-16.54-103.85q-19.62-19.61-19.62-47.69t19.62-47.69q19.61-19.62 47.69-19.62t47.69 19.62q19.62 19.61 19.62 47.69t-19.62 47.69q-19.61 19.62-47.69 19.62t-47.69-19.62ZM260-100v-161.23q-57-52-88.5-119.19-31.5-67.2-31.5-139.96 0-141.54 99.23-240.58Q338.46-860 480-860q116.54 0 208.42 69.66 91.89 69.65 119.35 180.73L865.23-380H740v200H580v80h-60v-140h160v-200h108l-38-155q-23-91.38-98.38-148.19Q576.23-800 480-800q-116 0-198 81.19t-82 197.19q0 60 24.5 113.81T294-312l26 24v188h-60Zm234-350Z" />
    </svg>
  );
}
